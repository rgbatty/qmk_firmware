...
The base layer has both halves of the layout joined with separate tables for the
alphas and thumbs.  Other layers are specified as a single hand including
thumbs.
...
included with the alphas.  The thumb keys are backspace, enter, delete on the right
and space, tab, escape on the left.
...
results = ''
for tap_row, hold_row in map(None, alphas_table + thumbs_table, hold_table):
  results += '    '
